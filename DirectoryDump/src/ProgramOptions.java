...
public class ProgramOptions {
	...
	public boolean parse() {
		...
		if (!this.haveSourceDir || !this.haveDestDir) {
			this.errorMessage = String.valueOf(this.errorMessage)
					+ "Deben pasarse los par\u00e1metros 'origen' (-s o --source) y 'destino' (-d o --dir). No todos los par\u00e1metros han sido pasados como argumento";
		}
		if (this.haveSourceDir && this.haveDestDir) {
			return true;
		}
	...
	private boolean checkSourceDirectoryOption(String argument, int argumentIndex, int numberOfArguments) {
		...
		if (!this.thereAreMoreArguments(numberOfArguments, argumentIndex)) {
			this.errorMessage = String.valueOf(this.errorMessage)
					+ "El parametro {0} debe ser seguido por el nombre del directorio de origen\n" + argument;
			salida = false;
		}
